import * as React from "react";
import type { SVGProps } from "react";
const JetBrainsFleet = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    fill="none"
    viewBox="0 0 70 70"
    width="100%"
    height="100%"
    preserveAspectRatio="xMidYMid meet"
    {...props}
  >
    <defs>
      <radialGradient
        id="prefix__a"
        cx={0}
        cy={0}
        r={1}
        gradientTransform="matrix(22.35433 -20.58122 27.17129 29.51214 38.648 42.538)"
        gradientUnits="userSpaceOnUse"
      >
        <stop offset={0.771} stopColor="#001AFF" />
        <stop offset={1} stopColor="#8ACEFF" />
      </radialGradient>
      <radialGradient
        id="prefix__b"
        cx={0}
        cy={0}
        r={1}
        gradientTransform="rotate(-30.543 79.837 -70.068)scale(16.777 22.1489)"
        gradientUnits="userSpaceOnUse"
      >
        <stop offset={0.719} stopColor="#FA00FF" stopOpacity={0} />
        <stop offset={1} stopColor="#FF00D6" stopOpacity={0.44} />
      </radialGradient>
      <radialGradient
        id="prefix__c"
        cx={0}
        cy={0}
        r={1}
        gradientTransform="rotate(49.385 -19.814 41.858)scale(47.8852)"
        gradientUnits="userSpaceOnUse"
      >
        <stop offset={0.026} stopColor="#8DFDFD" />
        <stop offset={0.271} stopColor="#87FBFB" />
        <stop offset={0.484} stopColor="#74D6F4" />
        <stop offset={0.932} stopColor="#0038FF" />
      </radialGradient>
      <radialGradient
        id="prefix__d"
        cx={0}
        cy={0}
        r={1}
        gradientTransform="rotate(137.237 9.434 23.195)scale(32.8316)"
        gradientUnits="userSpaceOnUse"
      >
        <stop offset={0.267} stopColor="#0500FF" stopOpacity={0} />
        <stop offset={1} stopColor="#0500FF" stopOpacity={0.15} />
      </radialGradient>
      <radialGradient
        id="prefix__e"
        cx={0}
        cy={0}
        r={1}
        gradientTransform="rotate(75.198 -4.629 32.631)scale(51.1484)"
        gradientUnits="userSpaceOnUse"
      >
        <stop offset={0.42} stopColor="#FF00E5" stopOpacity={0} />
        <stop offset={0.774} stopColor="#FF00F5" stopOpacity={0.64} />
        <stop offset={0.899} stopColor="#BE46FF" stopOpacity={0.87} />
      </radialGradient>
      <radialGradient
        id="prefix__g"
        cx={0}
        cy={0}
        r={1}
        gradientTransform="matrix(2.73484 22.75837 -34.39872 4.13365 29.458 35.276)"
        gradientUnits="userSpaceOnUse"
      >
        <stop offset={0} stopColor="#00B2FF" />
        <stop offset={0.571} stopColor="#74C5FF" />
        <stop offset={0.979} stopColor="#9FD7FF" />
      </radialGradient>
      <linearGradient
        id="prefix__f"
        x1={11.644}
        x2={82.363}
        y1={42.432}
        y2={43.401}
        gradientUnits="userSpaceOnUse"
      >
        <stop offset={0.432} stopColor="#FE62EE" stopOpacity={0} />
        <stop offset={0.818} stopColor="#FD3AF5" stopOpacity={0.47} />
      </linearGradient>
      <linearGradient
        id="prefix__h"
        x1={33.054}
        x2={37.35}
        y1={23.191}
        y2={49.344}
        gradientUnits="userSpaceOnUse"
      >
        <stop offset={0.042} stopColor="#0038FF" />
        <stop offset={0.724} stopColor="#48BFF1" stopOpacity={0.59} />
        <stop offset={1} stopColor="#74C5FF" stopOpacity={0} />
      </linearGradient>
    </defs>
    <path
      fill="url(#prefix__a)"
      d="M65.153 30.85c0 9.496-10.163 17.194-22.7 17.194S19.754 40.346 19.754 30.85s10.163-17.194 22.7-17.194 22.699 7.698 22.699 17.194"
    />
    <path
      fill="url(#prefix__b)"
      d="M65.153 30.85c0 9.496-10.163 17.194-22.7 17.194S19.754 40.346 19.754 30.85s10.163-17.194 22.7-17.194 22.699 7.698 22.699 17.194"
    />
    <path
      fill="url(#prefix__c)"
      d="M66 35c0 17.12-13.88 31-31 31S4 52.12 4 35 17.88 4 35 4c8.046 3.642 16.464 17.194 19.99 21.429 3.524 4.235 12.648 9.571 8.176-1.623C65.073 26.832 66 31.852 66 35"
    />
    <path
      fill="url(#prefix__d)"
      d="M66 35c0 17.12-13.88 31-31 31S4 52.12 4 35 17.88 4 35 4c8.046 3.642 16.464 17.194 19.99 21.429 3.524 4.235 12.648 9.571 8.176-1.623C65.073 26.832 66 31.852 66 35"
    />
    <path
      fill="url(#prefix__e)"
      d="M66 35c0 17.12-13.88 31-31 31S4 52.12 4 35 17.88 4 35 4c8.046 3.642 16.464 17.194 19.99 21.429 3.524 4.235 12.648 9.571 8.176-1.623C65.073 26.832 66 31.852 66 35"
    />
    <path
      fill="url(#prefix__f)"
      d="M66 35c0 17.12-13.88 31-31 31S4 52.12 4 35 17.88 4 35 4c8.046 3.642 16.464 17.194 19.99 21.429 3.524 4.235 12.648 9.571 8.176-1.623C65.073 26.832 66 31.852 66 35"
    />
    <path
      fill="url(#prefix__g)"
      d="M56.651 39.682c1.658 7.764-6.511 16.089-18.246 18.594s-22.59-1.757-24.248-9.52c-1.658-7.764 6.511-16.089 18.246-18.594s22.59 1.757 24.248 9.52"
    />
    <path
      fill="url(#prefix__h)"
      d="M56.651 39.682c1.658 7.764-6.511 16.089-18.246 18.594s-22.59-1.757-24.248-9.52c-1.658-7.764 6.511-16.089 18.246-18.594s22.59 1.757 24.248 9.52"
    />
    <path
      fill="#D6F8F8"
      fillOpacity={0.19}
      fillRule="evenodd"
      d="M51.462 49.883c3.074-3.133 4.386-6.66 3.698-9.882-.688-3.223-3.326-5.907-7.411-7.51-4.073-1.6-9.412-2.037-15.028-.838s-10.31 3.779-13.375 6.901c-3.074 3.133-4.386 6.66-3.698 9.883s3.326 5.906 7.412 7.51c4.072 1.6 9.41 2.037 15.027.838 5.616-1.2 10.31-3.779 13.375-6.902m-13.057 8.393c11.735-2.505 19.904-10.83 18.246-18.594-1.658-7.763-12.514-12.026-24.248-9.52-11.735 2.505-19.904 10.83-18.246 18.593 1.658 7.764 12.514 12.026 24.248 9.521"
      clipRule="evenodd"
    />
  </svg>
);
export default JetBrainsFleet;
